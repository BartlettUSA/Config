---
description: Create conventional commit with semantic message
...
## Inputs
- $ARGUMENTS: Optional specific files to stage (default: all staged changes)
...
2. Run `git diff --cached` to analyze staged changes (or `git diff` if nothing staged)
3. Determine the primary change type from the diff
4. Generate commit message: `<emoji> <type>(<scope>): <description>`
5. If $ARGUMENTS specified, stage those files first
6. Execute `git commit -m "<message>"`
7. Show the resulting commit with `git log -1 --oneline`
